...
def food():
    ...
    print(f"phone number and Emergency number is {phonevalue.get(),emergencyvalue.get()}")
    ...
    with open("pt_91_GUI_tut_12.txt","a") as f:
        f.writelines(f"""Name is {namevalue.get()}"
    phone number and Emergency number is {phonevalue.get(),emergencyvalue.get()}
    Gender is {gendervalue.get()}
    Payment mode is {pmodevalue.get()}
    food service is {foodservicevalue.get()}
    Value is submitted!!\n\n""")
...
pmodevalue=IntVar()
foodservicevalue=StringVar()#checkbox
